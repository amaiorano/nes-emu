...
namespace {
    bool ReadInputDown(uint16 controllerIndex, ControllerButtons::Type button) {
        static SDL_Scancode buttonMapping[] = {
            ...
            SDL_SCANCODE_S,    SDL_SCANCODE_A,     SDL_SCANCODE_TAB, SDL_SCANCODE_RETURN};
        ...
        // Input::GetScancodeName(buttonMapping[button]), ControllerButtons::Names[button]);
...
uint8 ControllerPorts::HandleCpuRead(uint16 cpuAddress) {
    ...
    static const ControllerButtons::Type reportOrder[] = {
    ...
    if (readIndex < ARRAYSIZE(reportOrder)) {
        ...
        if ((button == Down && lastIsButtonDown[Up]) ||
            (button == Right && lastIsButtonDown[Left])) {
            isButtonDown = false;
        }
    ...
    //  In the NES and Famicom, the top three (or five) bits are not driven, and so retain the bits
    //  of the previous byte on the bus. Usually this is the most significant byte of the address of
    //  the controller port—0x40. Paperboy relies on this behavior and requires that reads from the
    //  controller ports return exactly $40 or $41 as appropriate.
    //@TODO: For now, will just hack it to assume this was the last bus value. Eventually could do
    // this correctly in CpuMemoryBus
