...
// Some retail games overflow (on purpose?) like Battletoads so we can't leave this on
#define FAIL_ON_STACK_OVERFLOW 0

namespace {
    OpCodeEntry** g_opCodeTable = GetOpCodeTable();

    FORCEINLINE uint16 GetPageAddress(uint16 address) { return (address & 0xFF00); }
    ...
    FORCEINLINE uint8 CalcZeroFlag(uint8 v) { return v == 0; }
...
void Cpu::Execute(uint32& cpuCyclesElapsed) {
    ...
    ExecutePendingInterrupts(); // Handle when interrupts are called "between" CPU updates (e.g. PPU
                                // sends NMI)
...
void Cpu::HandleCpuWrite(uint16 cpuAddress, uint8 value) {
    ...
    case CpuMemory::kSpriteDmaReg: { // $4014
        ...
        static auto SpriteDmaTransfer = [&](uint16 cpuAddress) {
            ...
            // While DMA transfer occurs, the memory bus is in use, preventing CPU from fetching
            // memory
            m_cycles += 512;
        };
        ...
        // Note: we perform the full DMA transfer right here instead of emulating the transfers over
        // multiple frames. If we need to do it right, see
        // http://wiki.nesdev.com/w/index.php/PPU_programmer_reference#DMA
...
void Cpu::UpdateOperandAddress() {
    ...
    case AddressMode::IdxInd: {
        uint16 low = TO16((Read8(PC + 1) + X)) & 0x00FF; // Zero page low byte of operand address,
...
void Cpu::ExecuteInstruction() {
    ...
    case BIT: { // Test bits in memory with accumulator
        ...
        P.SetValue((P.Value() & 0x3F) | (memValue & 0xC0)); // Copy bits 6 and 7 of mem value to
                                                            // status register
        P.Set(Zero, CalcZeroFlag(result));
    } break;
...
uint8 Cpu::GetAccumOrMemValue() const {
    assert(m_opCodeEntry->addrMode == AddressMode::Accumu ||
           m_opCodeEntry->addrMode & AddressMode::MemoryValueOperand);
...
void Cpu::PushProcessorStatus(bool softwareInterrupt) {
    assert(!P.Test(StatusFlag::Unused) && !P.Test(StatusFlag::BrkExecuted) &&
           "P should never have these set, only on stack");
