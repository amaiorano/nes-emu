...
namespace StatusFlag {
    enum Type : uint8 {
        ...
        BrkExecuted = BIT(4), // BRK executed (IRQ/software interupt) *NOTE: Not actually a bit in
                              // P, only set on stack for s/w interrupts
...
private:
    ...
    // Updates m_operandAddress for current instruction based on addressing mode. Operand data is
    // assumed to be at PC + 1 if it exists.
