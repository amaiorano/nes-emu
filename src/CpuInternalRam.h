...
public:
    ...
    void Serialize(class Serializer& serializer) { SERIALIZE(m_memory); }

    uint8 HandleCpuRead(uint16 cpuAddress) {
        return m_memory.Read(MapCpuToInternalRam(cpuAddress));
    }
    void HandleCpuWrite(uint16 cpuAddress, uint8 value) {
        m_memory.Write(MapCpuToInternalRam(cpuAddress), value);
    }
