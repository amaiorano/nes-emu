...
#define TRACE(text)                             \
    do {                                        \
        strcat(Trace::g_curr, text);            \
        Trace::g_curr += (ARRAYSIZE(text) - 1); \
    } while (false)
...
public:
    ...
    void Initialize(Nes& nes) { m_nes = &nes; }

    void Shutdown() { Trace::Close(); }
...
private:
    template <typename T>
    void MemoryDump(T& memoryBus, FileStream& fs, uint16 start = 0x0000, size_t size = 0,
    ...
    void MemoryDumpPpu(const std::string& dumpDir, PpuMemoryBus& ppuMemoryBus) {
        ...
        for (size_t i = 0; i < PpuMemory::kNumPatternTables; ++i) {
            ...
            MemoryDump(ppuMemoryBus, fs, PpuMemory::GetPatternTableAddress(i),
                       PpuMemory::kPatternTableSize, 32 * 2);
        }

        for (size_t i = 0; i < PpuMemory::kNumMaxNameTables; ++i) {
            fs.Printf("\nName Table %d (%d bytes)\n\n", i, PpuMemory::kNameTableSize);
            MemoryDump(
                ppuMemoryBus, fs, PpuMemory::GetNameTableAddress(i), PpuMemory::kNameTableSize, 32);

            fs.Printf("\nAttribute Table %d (%d bytes)\n\n", i, PpuMemory::kAttributeTableSize);
            MemoryDump(ppuMemoryBus, fs, PpuMemory::GetAttributeTableAddress(i),
                       PpuMemory::kAttributeTableSize, 32);
        }
...
#define HILO(v)                                                     \
    (cpu.P.Test(v) ? StatusFlagNames[BitFlagToPos<v>::Result - 1] : \
                     tolower(StatusFlagNames[BitFlagToPos<v>::Result - 1]))
#define ADDR_8_NO$ "%02X"

        TRACEF("A:" ADDR_8_NO$ " X:" ADDR_8_NO$ " Y:" ADDR_8_NO$ " S:" ADDR_8_NO$ " P:%c%c%c%c%c%c%"
               "c%c",
               cpu.A, cpu.X, cpu.Y, cpu.SP, HILO(Negative), HILO(Overflow), HILO(Unused),
               HILO(BrkExecuted), HILO(Decimal), HILO(IrqDisabled), HILO(Zero), HILO(Carry));
...
#undef ADDR_8_NO$
    ...
    void PrintInstruction() {
        ...
        const uint16 operandAddress = cpu.m_operandAddress; // Expected to be updated for current
                                                            // instruction
...
// For branch instructions, resolve the target address and print it in comments
            ...
            sprintf(
                operandText, ADDR_8 " ; " ADDR_16 " (%d)", (uint8)offset, operandAddress, offset);
#else
        ...
        case AddressMode::ZeroPg: {
            //@TODO: Do zero-page instructions really specify a 16 bit address: $00xx? This is what
            // fceux outputs...
        ...
        case AddressMode::ZPIdxX: {
            ...
            sprintf(operandText, ADDR_8 ",X @ " ADDR_16 " = #" ADDR_8, address, operandAddress,
                    cpu.Read8(operandAddress));
        } break;

        case AddressMode::ZPIdxY: {
            const uint8 address = cpu.Read8(PC + 1);
            sprintf(operandText, ADDR_8 ",Y @ " ADDR_16 " = #" ADDR_8, address, operandAddress,
                    cpu.Read8(operandAddress));
        } break;
        ...
        case AddressMode::Absolu: {
            ...
            if (isJump)
                ...
                sprintf(
                    operandText, ADDR_16 " = #" ADDR_8, operandAddress, cpu.Read8(operandAddress));
        } break;

        case AddressMode::AbIdxX: {
            const uint16 address = cpu.Read16(PC + 1);
            sprintf(operandText, ADDR_16 ",X @ " ADDR_16 " = #" ADDR_8, address, operandAddress,
                    cpu.Read8(operandAddress));
        } break;

        case AddressMode::AbIdxY: {
            const uint16 address = cpu.Read16(PC + 1);
            sprintf(operandText, ADDR_16 ",Y @ " ADDR_16 " = #" ADDR_8, address, operandAddress,
                    cpu.Read8(operandAddress));
        } break;

        case AddressMode::Indrct: {
            const uint16 address = cpu.Read16(PC + 1);
            sprintf(operandText, "(" ADDR_16 ") @ " ADDR_16 " = #" ADDR_8, address, operandAddress,
                    cpu.Read8(operandAddress));
        } break;
    ...
    void ProcessBreakpoints() {
        ...
        {
            auto iter = std::find(
                std::begin(g_instructionBreakpoints), std::end(g_instructionBreakpoints), cpu.PC);
        ...
        {
            auto iter = std::find(
                std::begin(g_dataBreakpoints), std::end(g_dataBreakpoints), cpu.m_operandAddress);
...
namespace Debugger {
    ...
    void DumpMemory() {
        ScopedExecuting se;
        g_debugger.DumpMemory();
    }
    void PreCpuInstruction() {
        ScopedExecuting se;
        g_debugger.PreCpuInstruction();
    }
    void PostCpuInstruction() {
        ScopedExecuting se;
        g_debugger.PostCpuInstruction();
    }
