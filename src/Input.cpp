...
namespace Input {
    ...
    bool AltDown() { return KeyDown(SDL_SCANCODE_LALT) || KeyDown(SDL_SCANCODE_RALT); }

    bool CtrlDown() { return KeyDown(SDL_SCANCODE_LCTRL) || KeyDown(SDL_SCANCODE_RCTRL); }

    bool ShiftDown() { return KeyDown(SDL_SCANCODE_LSHIFT) || KeyDown(SDL_SCANCODE_RSHIFT); }

    const char* GetScancodeName(SDL_Scancode scanCode) { return SDL_GetScancodeName(scanCode); }
}
