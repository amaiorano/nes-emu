...
// Mapper is used to map cartridge (physical) memory banks to CPU/PPU (virtual) memory banks. A bank
// is a chunk of memory of fixed size (e.g. 4K for CPU). In the virtual address space of the
// CPU/PPU, the number of banks are limited, but there may be more physical banks on the /
// cartridge; thus the mapper's job is to detect when it needs to remap (switch) certain banks by
// snooping for specific writes on the CPU/PPU memory bus.
