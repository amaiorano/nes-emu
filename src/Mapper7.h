...
public:
    virtual const char* MapperName() const { return "AxROM"; }

    virtual void PostInitialize() { SetPrgBankIndex32k(0, 0); }
    ...
    virtual void OnCpuWrite(uint16 cpuAddress, uint8 value) {
        if (cpuAddress >= 0x8000) {
            ...
            SetNameTableMirroring(selectOneScreenLower ? NameTableMirroring::OneScreenLower :
