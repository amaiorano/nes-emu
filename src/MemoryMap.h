...
namespace CpuMemory {
    ...
    const uint16 kControllerPort1 = 0x4016; // (RW) Strobe for both controllers (bit 0), and
                                            // controller 1 output
...
namespace PpuMemory {
    ...
    // two, either horizontally or vertically, or the cart supplies and extra 2 KB memory for 4
    // screen. Also, a "name table" includes the attribute table, which are in the last 64 bytes.
