...
#define SERIALIZE_BUFFER(buffer, size) \
    serializer.SerializeBuffer(#buffer, reinterpret_cast<uint8*>(buffer), size)
...
public:
    ...
    void SerializeValue(const char* name, T& value) {
        // This catches types with vtables, non-trivial copy constructors and assignment operators,
        // etc. But we can't determine if the type aggregates a pointer, which would be a problem.
        static_assert(std::is_trivially_copyable<T>::value,
                      "Type must be trivially copyable to "
                      "serialize");
        ...
        if (m_saving) {
            ...
            if (nameFromFile.compare(name) != 0)
                FAIL("SaveState data mismatch! Looking for %s, found %s", name,
                     nameFromFile.c_str());
            ReadValue(value);
        }
    }
