...
const char* kVersionString =
    "v" STRINGIZE(kVersionMajor) "." STRINGIZE(kVersionMinor) "." STRINGIZE(kVersionMinor2)
        kVersionConfig;

namespace {
    void PrintAppInfo() {
        const char* text = "### %s %s - Nintendo Entertainment System Emulator\n"
    ...
    bool OpenRomFileDialog(std::string& fileSelected) {
        return System::SupportsOpenFileDialog() &&
               System::OpenFileDialog(
                   fileSelected, "Open NES rom", FILE_FILTER("NES Rom", "*.nes"));
    }
    ...
    void ProcessInputForChannelVolumes(Nes& nes) {
        ...
        static ChannelState apuChannelState[] = {
            {ApuChannel::Pulse1, SDL_SCANCODE_F1, true}, // We assume all channels are on (1.0f) by
                                                         // default
...
int main(int argc, char* argv[]) {
    try {
        ...
        while (!quit) {
            ...
            Renderer::SetWindowTitle(FormattedString<>("%s %s [FPS: %2.2f] %s", APP_NAME,
                                                       kVersionString, nes->GetFps(),
                                                       paused ? "*PAUSED*" : "")
                                         .Value());
            ...
            if (Input::KeyPressed(SDL_SCANCODE_LEFTBRACKET) ||
                Input::KeyDown(SDL_SCANCODE_RIGHTBRACKET)) {
                stepOneFrame = true;
